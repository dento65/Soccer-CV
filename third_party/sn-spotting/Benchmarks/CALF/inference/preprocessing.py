...
def timestamps2long(output_spotting, video_size, chunk_size, receptive_field):
    ...
    if video_size < chunk_size:
        tmp = torch.zeros([chunk_size,output_spotting.size()[-1]-2], dtype=torch.float, device=output_spotting.device)-1
        for i in np.arange(output_spotting.size()[1]):
            tmp[torch.floor(output_spotting[0,i,1]*(chunk_size-1)).type(torch.int), torch.argmax(output_spotting[0,i,2:]).type(torch.int)] = output_spotting[0,i,0]
        return tmp[:video_size]
...
def batch2long(output_segmentation, video_size, chunk_size, receptive_field):
    ...
    if video_size < chunk_size:
        return (1-output_segmentation[0])[:video_size]
