...
class SoccerNetClipsTesting(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # The released benchmark assumes at least one 120 s chunk. For short
        # uploads, repeat the final feature only for model padding; retain the
        # original size so post-processing crops predictions to the video.
        if size < self.chunk_size:
            feat_half1 = np.concatenate([feat_half1, np.repeat(feat_half1[-1:], self.chunk_size - size, axis=0)], axis=0)
