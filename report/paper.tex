...
\maketitle
\begin{abstract}
Football highlights must preserve an event's build-up and outcome while limiting viewing time. PitchClipers combines pretrained action spotting, asymmetric temporal decoding, a duration-budgeted policy and player--ball evidence. We formulate context-based evaluation and implement a reproducible decoder. A controlled experiment removes 32.7\% duplicate viewing time through interval merging, motivating separate measurement of action confidence and contextual sufficiency.
\end{abstract}
...
\textbf{Temporal proposals and decoding.} The released CALF pipeline uses ResNet-152 descriptors, PCA reduction and context-aware action spotting for 17 action classes. Each proposal is $(t_i,c_i,s_i)$: timestamp, class and confidence. A confidence-filtered proposal generates
\begin{equation}
 I_i=[\max(0,t_i-b),\min(T,t_i+a)],
\end{equation}
where $T$ is video duration. The default $b=8$ and $a=5$ seconds retain asymmetric context. Overlapping intervals are merged to avoid repeating footage. We also implement a deterministic greedy budget policy that selects proposals by score per added union second. Users inspect proposal provenance before export; fixed windows provide a reproducible baseline.

\textbf{Spatial evidence.} YOLO11 detections and ByteTrack~\cite{bytetrack} provide trajectories, ball visibility and nearest-player proximity cues. Missing ball observations remain explicit. A second right-most image coordinate is shown only as a screen-position guide; it is not a rules-based offside decision.
...
\end{equation}
Coverage $R$ measures retained context; precision $P$ penalizes unrelated footage. The evaluator reports their harmonic mean, complete-context rate, union viewing duration and redundant seconds removed by merging. It applies to any spotter given shared context annotations, which timestamp labels alone cannot supply. Scores are deliberately withheld for the present pilot because no independent context annotations are available.
...
\textbf{Operational pilot.} On a supplied 30-second wide-view broadcast clip, YOLO11+ByteTrack processed 750 frames in 107.4 seconds. It produced 6.2 players per sampled frame, while the small ball appeared in 1/750 frames and yielded one proximity-control cue. A cached repeat returned in 0.003 seconds. The result shows why spatial evidence must expose missing ball observations instead of turning them into a possession claim. An exported H.264 highlight reel played for 30.0 seconds after verification.

Additional footage can contain either useful build-up or unrelated play; duration alone cannot measure highlight quality. These local sensitivity and operational results establish decoder behaviour and execution, not spotting accuracy. Evaluating contextual sufficiency requires held-out interval annotations and equal viewing budgets.

\textbf{Conclusion.} The interactive demo exposes event timelines, policy provenance, tracking overlays and MP4 reels. The next experiment compares fixed decoding and learned selection, with and without tracking features, using context scores at equal viewing duration.
